...
function inicializarDashboardCoordenador(user) {
  ...
  // No modo normal, o aviso da tela inicial aparece apenas uma vez no navegador.
  const MODAL_BOAS_VINDAS_KEY = 'sistema_faltas_modal_inicial_normal_visto';
  if (criticos > 0 && localStorage.getItem(MODAL_BOAS_VINDAS_KEY) !== 'sim') {
    document.getElementById('modal-count').textContent = criticos;
    setTimeout(() => {
      document.getElementById('modal-boas-vindas').classList.add('aberto');
      localStorage.setItem(MODAL_BOAS_VINDAS_KEY, 'sim');
    }, 400);
  }
